...
public class GAgentService : IGAgentService
{
    ...
    public async Task<Dictionary<GrainType, List<Type>>> GetAllAvailableGAgentInformation()
    {
        ...
        try
        {
            ...
            var gAgentTypes = _gAgentManager.GetAvailableGAgentTypes();
            ...
            foreach (var gAgentType in gAgentTypes)
            {
                ...
                {
                    // Get GrainType
                    var grainType = _grainTypeResolver.GetGrainType(gAgentType);
                    
                    // Create temporary instance to get event types
                    var tempGAgent = await _gAgentFactory.GetGAgentAsync(gAgentType);
                    ...
                    if (subscribedEvents != null && subscribedEvents.Any())
                    {
                        ...
                        {
                            gAgentInfo[grainType] = new List<Type>();
                        }
