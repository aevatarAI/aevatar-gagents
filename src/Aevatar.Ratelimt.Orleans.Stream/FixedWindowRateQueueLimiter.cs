...
namespace Aevatar.Ratelimt.Orleans.Stream;
...
/// </summary>
public class FixedWindowRateStreamLimiterGrain<TLimiter> : Grain where TLimiter : RateLimiter
{

    private const string NameSpace = $"FixedWindowRateStreamLimiter";
    protected RateLimiter RateLimiter { get; set; }

    // private readonly Queue<RequestRegistration> _queue = new Queue<RequestRegistration>();


    private IAsyncStream<RequestRegistration> _stream;
    private object Lock => _stream;


    private IStreamProvider StreamProvider => this.GetStreamProvider(NameSpace);
    ...
    protected RateLimitLease AttemptAcquireCore(int permitCount)
    {
        throw new NotImplementedException();
    }

    public FixedWindowRateStreamLimiterGrain(FixedWindowRateLimiterOptions options)
    {
        if (options is null)
        {
    ...
    public override async Task OnActivateAsync(CancellationToken cancellationToken)
    {
        ...
        // Create a stream with a unique StreamId and namespace
        var streamId = StreamId.Create(NameSpace, this.GetGrainId().ToString());
        _stream = StreamProvider.GetStream<RequestRegistration>(streamId);
        await _stream.SubscribeAsync(Handler);
    }

    private Func<RequestRegistration, StreamSequenceToken, Task> Handler =>
        async (message, token) => { ReplenishInternal(message, token); };

    private async Task EnqueueRequestAsync(RequestRegistration request)
    {
        // Push data into the stream
        await _stream.OnNextAsync(request);
    }

    // Used in tests that test behavior with specific time intervals
    private void ReplenishInternal(RequestRegistration requestRegistration, StreamSequenceToken cancellationToken)
    {
        ...
        lock (Lock)
        {
            ...
            var nextPendingRequest = requestRegistration;
    ...
    protected async ValueTask<RateLimitLease> AcquireAsyncCore(int permitCount,
...
#pragma warning disable CS4014
            EnqueueRequestAsync(registration);
#pragma warning restore CS4014
    ...
    public sealed class RequestRegistration : TaskCompletionSource<RateLimitLease>
    {
        ...
        public RequestRegistration(int permitCount, FixedWindowRateStreamLimiterGrain<TLimiter> limiter,
            CancellationToken cancellationToken)
        ...
        private static void Cancel(object? state)
        {
            ...
            {
                var limiter = (FixedWindowRateStreamLimiterGrain<TLimiter>)registration.Task.AsyncState!;
                lock (limiter.Lock)
                {
